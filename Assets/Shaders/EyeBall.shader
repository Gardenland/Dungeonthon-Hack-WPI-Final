Shader "Custom/EyeBall" {
	...
	SubShader {
		...
		struct Input {
         ...
         v.vertex.z += .1 * sin( 50 *_Time + v.vertex.y * 5) * v.texcoord.y;
          v.vertex.x += .1 * sin( 50 *_Time + v.vertex.y * 5) * v.texcoord.y;

         v.vertex.y += .1 * sin(_Time * 10 );
          //v.vertex.xyz *= 1- v.texcoord.y;
           //v.vertex.z -= .5 * abs(sin(20*_Time + v.vertex.z * 3 )) * v.texcoord.y;
